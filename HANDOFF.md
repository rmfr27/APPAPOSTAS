...
- **Tips diárias (idempotente — 1 par por dia)**: "Segura" reutiliza `generateCombo(events, 'segura', 4)` sem alterações. "Arriscada" **não** reutiliza `generateCombo(events, 'valor', N)` diretamente — descoberto em teste: o pool "valor" pode combinar mercados finos/pouco cotados (ex. "Empate" em boxe a 33.00 numa só casa) e produzir odds totais absurdas (testei e apanhei uma combinação de **205 milhões**). Resolvido com `buildRiskyCombo` em `bankroll.js`, que filtra pernas com odd > 8 antes de montar o combinado — mantém-se "arriscado" (odds totais na casa dos milhares para 7 pernas é normal e esperado) sem ser degenerado.
